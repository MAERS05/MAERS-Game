...
export function resolve(p1Ctx, p2Ctx, p1State, p2State, bothInsighted, turn) {
  ...
  const timeline = [];
  _addEvents(timeline, PlayerId.P1, p1Ctx);
  _addEvents(timeline, PlayerId.P2, p2Ctx);
  ...
  const log = _executeTimeline(timeline, bs);

  // ── 3. 从日志涌现推断情形，含处决覆盖 ────────────────────
  const derived = _deriveClash(
    log, p1Ctx, p2Ctx, p1State, p2State,
...
function _addEvents(timeline, playerId, ctx) {
  ...
  switch (ctx.action) {
    ...
    case Action.DODGE:
      // 闪避的比较维度是速度，pts === speed（engine 层保证）
      timeline.push({
 ...
 */
function _executeTimeline(timeline, bs) {
  const log = [];
...
function _resolveSingleAttack(attack, bs, log) {
  ...
  if (target.evasions.length > 0) {
    const best = target.evasions.reduce((b, e) => e.pts > b.pts ? e : b);
    if (best.pts >= attack.speed) {
      log.push({ kind: 'EVADE',        attackerId: attack.actorId, dodgerId:   attack.targetId, atkSpeed: attack.speed, dodgeSpeed: best.pts });
    } else {
      bs[attack.targetId].dmgReceived += 1;
      log.push({ kind: 'SWIFT_STRIKE', attackerId: attack.actorId, dodgerId:   attack.targetId, atkSpeed: attack.speed, dodgeSpeed: best.pts });
    }
    return;
  }
  ...
  // 无任何防御：命中
  // （目标选了守备/闪避但速度不足时，其 buff 在此速度 tick 之后才挂载，所以此处依然"无防御"）
  bs[attack.targetId].dmgReceived += 1;
...
function _deriveClash(log, p1Ctx, p2Ctx, p1State, p2State, rawDmgP1, rawDmgP2) {
  ...
  if (p1Act === Action.GUARD && p2Act === Action.GUARD)
    return _zero(Clash.ACCUMULATE, '双方同时举起防御，战场陷入僵持——【蓄势】。');

  if (p1Act === Action.DODGE && p2Act === Action.DODGE)
    return _zero(Clash.RETREAT, '双方同时展开闪避，错身而过——【退让】。');

  if ((p1Act === Action.DODGE && p2Act === Action.GUARD) ||
      (p1Act === Action.GUARD && p2Act === Action.DODGE))
    return _zero(Clash.PROBE, '一方试图闪开，另一方缩于防御——双方试探，无功而返。【试探】');
  ...
  if (p1Act !== Action.ATTACK && p2Act === Action.STANDBY)
    return _zero(Clash.WASTED_ACTION,
      `你发动了${ActionName[p1Act]}，而敌方处于待命——行动毫无意义。`);

  if (p2Act !== Action.ATTACK && p1Act === Action.STANDBY)
    return _zero(Clash.WASTED_ACTION,
      `敌方发动了${ActionName[p2Act]}，而你处于待命——行动毫无意义。`);
...
function _deriveFromLog(log, p1Ctx, p2Ctx, p1State, p2State, rawDmgP1, rawDmgP2) {
  ...
  const swiftStrike = log.find(e => e.kind === 'SWIFT_STRIKE');
  ...
  if (confront) {
    ...
    // 攻速 > 闪避速度：攻击抢在闪避挂载前命中
    const atkIsP1 = swiftStrike.attackerId === PlayerId.P1;
    clash     = Clash.SWIFT_STRIKE;
    clashDesc = atkIsP1
      ? `你的速度（${swiftStrike.atkSpeed}）超过敌方闪避（${swiftStrike.dodgeSpeed}），抢先命中！【迅攻】`
      : `敌方速度（${swiftStrike.atkSpeed}）超过你的闪避（${swiftStrike.dodgeSpeed}），抢先命中！【迅攻】`;
    ...
    clash     = Clash.EVADE;
    clashDesc = atkIsP1
      ? `敌方的闪避速度（${evade.dodgeSpeed}）不低于你的攻击速度（${evade.atkSpeed}），如鬼魅般躲开。【规避】`
      : `你的闪避速度（${evade.dodgeSpeed}）不低于敌方攻击速度（${evade.atkSpeed}），如残影般闪过。【规避】`;
    ...
    if (targetCtx.action === Action.GUARD) {
      ...
      clashDesc = atkIsP1 ? '攻击命中敌方。' : '攻击命中你。';
    }
